# predict.py (已更新：打印前50点误差 + 统计最大误差)
import torch
...
from dataset import PCBDataset, get_all_current_info, get_global_stats  # 确保 dataset.py 是最新版
from visualize import save_heatmap_plot
from itertools import product


def main():
    print("--- 运行预测与可视化 (可变长序列, -3.0V过滤, 0V修正) ---")
    ...
    test_board_id = random.choice(test_ids)
    ...
    max_error_raw = np.max(abs_errors)  # <--- 新增：计算最大误差

    print(f"\n--- 电路板 {test_board_id} 的【原始】评估结果 (过滤后数据) ---")
    print(f"  节点数: {features.shape[0]}")
    print(f"  平均绝对误差 (MAE): {mae_raw:.6f} V")
    # print(f"  最大绝对误差 (Max Error): {max_error_raw:.6f} V")  # <--- 新增：打印最大误差
    ...
    print(f"  最大绝对误差 (Max Error): {max_error_post_processed:.6f} V")  # <--- 新增
